...
cs_state_t BTSYMBOLS(bt_cs_get_state)(bt_instance_t* ins, bt_address_t* addr)
{
    ...
    memcpy(&packet.cs_pl._bt_cs_get_state.addr, addr, sizeof(bt_address_t));
    status = bt_socket_client_sendrecv(ins, &packet, BT_CS_GET_STATE);
    if (status != BT_STATUS_SUCCESS)
        return CS_STOPPED;

    return packet.cs_r.measure_state;
}
